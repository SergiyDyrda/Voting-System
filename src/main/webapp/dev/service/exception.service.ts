...
export class ExceptionService {
    ...
    onError(e) {
        ...
        if (error) {
            ...
            }else if (error.cause === 'TooLateToVoteException') {
                errorModel = new ErrorModel(this.i18Service.getMessage('exception.too_late'), null);
            } else if (error.cause === 'ValidationException') {
                errorModel = new ErrorModel(error.details, null);
            }
        }
